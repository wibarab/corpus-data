...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xtoks="http://acdh.oeaw.ac.at/xtoks" exclude-result-prefixes="#all" version="2.0" xml:base="postTokenization.xsl">
    ...
    <xsl:template name="groupTokenParts">
        <xsl:for-each-group group-starting-with="xtoks:ws" select="node()">
            ...
            <xsl:variable name="last-is-pc" select="exists(current-group()[last()]/self::xtoks:pc)" />
            <xsl:choose>
    ...
    <xsl:template match="xtoks:w[exists(following-sibling::*) and not(following-sibling::*[1]/self::xtoks:ws)]">
        ...
        <xsl:copy copy-namespaces="no">
            ...
            <xsl:attribute name="xtoks:id" select="if (matches($rawID,'^\d')) then concat('w',$rawID) else $rawID" />
